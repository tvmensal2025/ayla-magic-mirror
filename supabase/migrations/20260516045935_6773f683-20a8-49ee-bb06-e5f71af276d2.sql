...
BEGIN
  ...
  IF v_customer_id IS NULL AND _remote_jid IS NOT NULL THEN
    ...
    SELECT id INTO v_customer_id FROM customers
     WHERE consultant_id = _consultant_id AND phone_whatsapp = v_phone
  ...
  UPDATE crm_deals SET stage = 'novo_lead', approved_at = NULL, rejected_at = NULL,
    rejection_reason = NULL, updated_at = now()
    WHERE customer_id = v_customer_id;
  GET DIAGNOSTICS v_count = ROW_COUNT; v_deleted := v_deleted || jsonb_build_object('crm_deals_reset', v_count);
