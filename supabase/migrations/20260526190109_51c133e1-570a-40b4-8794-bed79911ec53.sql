-- M1: contador separado de perguntas livres à IA por passo.
ALTER TABLE public.customer_flow_state
  ADD COLUMN IF NOT EXISTS ai_questions_this_step int NOT NULL DEFAULT 0;

-- M5 (extensão da view de saúde): contagem de falhas de sync do CRM.
...
SELECT
  ...
  COALESCE(cs.crm_sync_errors_24h, 0)::bigint AS crm_sync_errors_24h
FROM consultants c
...
LEFT JOIN LATERAL (
  SELECT count(*) AS crm_sync_errors_24h
  FROM public.engine_logs el
  JOIN public.customers cu3 ON cu3.id = el.customer_id
  WHERE cu3.consultant_id = c.id
    AND el.kind = 'engine_crm_sync_failed'
    AND el.at > now() - interval '24 hours'
) cs ON true
GROUP BY c.id, c.name, c.flow_engine_v3, dl.dark_outputs_24h, dl.dark_output_errors_24h, cs.crm_sync_errors_24h;
