-- Hard reset: o botão Zerar agora apaga 100% dos rastros e o próprio customer.
...
BEGIN
  ...
  -- Normaliza telefone (aceita '11989000650', '5511989000650', '5511989000650@s.whatsapp.net')
  ...
  IF v_customer_id IS NULL THEN
    -- Mesmo sem customer, ainda apaga registros remote_jid puros
    IF v_remote_jid IS NOT NULL THEN
  ...
  DELETE FROM bot_flow_rule_fires WHERE customer_id = v_customer_id;
  ...
  -- Por último, apaga o próprio customer (hard delete)
...
-- Reset imediato dos dois números de teste
DO $$
DECLARE
  v_ids uuid[];
  v_jids text[] := ARRAY['5511989000650@s.whatsapp.net','5511971254913@s.whatsapp.net'];
BEGIN
  SELECT coalesce(array_agg(id), ARRAY[]::uuid[]) INTO v_ids
    FROM customers
    WHERE regexp_replace(coalesce(phone_whatsapp,''), '\D', '', 'g')
          IN ('5511989000650','5511971254913');
  ...
  IF array_length(v_ids,1) IS NOT NULL THEN
    ...
    DELETE FROM ai_agent_logs           WHERE customer_id = ANY(v_ids);
    DELETE FROM bot_step_transitions    WHERE customer_id = ANY(v_ids);
    DELETE FROM bot_flow_rule_fires     WHERE customer_id = ANY(v_ids);
    DELETE FROM bot_handoff_alerts      WHERE customer_id = ANY(v_ids);
    DELETE FROM customer_memory         WHERE customer_id = ANY(v_ids);
    DELETE FROM whatsapp_message_buffer WHERE customer_id = ANY(v_ids);
    DELETE FROM worker_phase_logs       WHERE customer_id = ANY(v_ids);
    DELETE FROM facebook_capi_events    WHERE customer_id = ANY(v_ids);
    DELETE FROM crm_deals               WHERE customer_id = ANY(v_ids);
  END IF;

  DELETE FROM whatsapp_message_buffer
    WHERE regexp_replace(coalesce(phone,''), '\D', '', 'g') IN ('5511989000650','5511971254913');
  DELETE FROM crm_deals               WHERE remote_jid = ANY(v_jids);
  DELETE FROM scheduled_messages      WHERE remote_jid = ANY(v_jids);
  DELETE FROM crm_auto_message_log    WHERE remote_jid = ANY(v_jids);
