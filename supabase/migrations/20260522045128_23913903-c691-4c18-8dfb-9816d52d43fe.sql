-- ============================================================================
-- WhatsApp Flow Reliability v2
-- Idempotent: safe to re-run.
-- ============================================================================
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- 4.1 inbound_media_failures
...
BEGIN
  SELECT until_at INTO v_until FROM public.ai_cooldown_state WHERE cooldown_key = p_key;
...
CREATE OR REPLACE FUNCTION public.consume_gemini_token(
  p_consultant UUID, p_tokens INT DEFAULT 1
) RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_row public.gemini_quota_bucket%ROWTYPE; v_elapsed_min NUMERIC; v_new_tokens INT;
BEGIN
  INSERT INTO public.gemini_quota_bucket (consultant_id) VALUES (p_consultant)
  ON CONFLICT (consultant_id) DO NOTHING;
  SELECT * INTO v_row FROM public.gemini_quota_bucket WHERE consultant_id = p_consultant FOR UPDATE;
  ...
  IF v_new_tokens < p_tokens THEN
    UPDATE public.gemini_quota_bucket SET tokens = v_new_tokens, refilled_at = now()
     WHERE consultant_id = p_consultant;
    RETURN false;
  END IF;
  UPDATE public.gemini_quota_bucket SET tokens = v_new_tokens - p_tokens, refilled_at = now()
   WHERE consultant_id = p_consultant;
  RETURN true;
END; $$;

-- 4.7 webhook_message_dedup
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_constraint
     WHERE conname = 'webhook_message_dedup_pkey'
       AND conrelid = 'public.webhook_message_dedup'::regclass) THEN
    ALTER TABLE public.webhook_message_dedup DROP CONSTRAINT webhook_message_dedup_pkey;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_constraint
     WHERE conname = 'webhook_message_dedup_message_id_key'
       AND conrelid = 'public.webhook_message_dedup'::regclass) THEN
    ALTER TABLE public.webhook_message_dedup DROP CONSTRAINT webhook_message_dedup_message_id_key;
  END IF;
END $$;
...
CREATE OR REPLACE FUNCTION public.reserve_media_send(
  p_cons UUID, p_cust UUID, p_media UUID,
  p_slot_key TEXT DEFAULT 'unknown', p_kind TEXT DEFAULT 'media'
) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_id UUID;
BEGIN
  IF p_cust IS NULL OR p_media IS NULL THEN RETURN NULL; END IF;
  ...
  DO UPDATE SET
    dispatch_status = CASE
      ...
      WHEN public.ai_slot_dispatch_log.reserved_at < now() - interval '30 seconds' THEN 'reserved'
    ...
    reservation_id = CASE
      ...
      WHEN public.ai_slot_dispatch_log.reserved_at IS NULL
        OR public.ai_slot_dispatch_log.reserved_at < now() - interval '30 seconds' THEN gen_random_uuid()
    ...
    reserved_at = CASE
      ...
      WHEN public.ai_slot_dispatch_log.reserved_at IS NULL
        OR public.ai_slot_dispatch_log.reserved_at < now() - interval '30 seconds' THEN now()
...
CREATE OR REPLACE FUNCTION public.confirm_media_send(p_res_id UUID, p_ok BOOLEAN)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF p_res_id IS NULL THEN RETURN; END IF;
...
-- 4.10 conversations.message_text_hash
...
-- 4.11 consultants.flow_reliability_v2
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint
     WHERE conname = 'consultants_flow_reliability_v2_chk'
       AND conrelid = 'public.consultants'::regclass) THEN
    ALTER TABLE public.consultants
...
COMMENT ON COLUMN public.consultants.flow_reliability_v2 IS
  'Rollout flag for the WhatsApp Flow Reliability v2 bugfix. Values: off, dark, canary, on.';

-- 4.12 customer_processing_lock
...
CREATE OR REPLACE FUNCTION public.try_acquire_customer_lock(p_customer UUID, p_ttl_ms INT)
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_token UUID;
BEGIN
  IF p_customer IS NULL THEN RETURN NULL; END IF;
  v_token := gen_random_uuid();
  INSERT INTO public.customer_processing_lock (customer_id, locked_at, locked_until, lock_token)
  VALUES (p_customer, now(), now() + (p_ttl_ms || ' milliseconds')::interval, v_token)
  ...
  UPDATE public.customer_processing_lock
     SET locked_at = now(),
         ...
         lock_token = v_token
   WHERE customer_id = p_customer AND locked_until < now();
  IF FOUND THEN RETURN v_token; END IF;
  RETURN NULL;
END; $$;

CREATE OR REPLACE FUNCTION public.release_customer_lock(p_customer UUID, p_token UUID)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF p_customer IS NULL OR p_token IS NULL THEN RETURN false; END IF;
  DELETE FROM public.customer_processing_lock
   WHERE customer_id = p_customer AND lock_token = p_token;
  RETURN FOUND;
END; $$;