-- ════════════════════════════════════════════════════════════════════
-- reset_lead_conversation: também apaga customer_flow_state
-- ════════════════════════════════════════════════════════════════════
--
-- Problema: o RPC atual deleta o customer mas deixa customer_flow_state
-- órfão. Quando um lead novo é criado (mesmo telefone), o trigger
-- trg_create_customer_flow_state insere uma linha NEW para o novo
-- customer_id, mas a antiga continua com flow_id/current_step_id estale.
-- Para os leads em flow_variant='D' isso não causa erro hoje, mas
-- assim que o engine v3 sair de 'dark', loadContext pode encontrar
-- esses resíduos. Limpar agora é defensivo e barato.
--
-- Também garante que "Zerar conversa" do botão da UI restaure o lead
-- 100% para começar o flow do zero — sem pré-seleção de step herdada.
--
-- Idempotente: CREATE OR REPLACE; mesma assinatura; sem efeito sobre
-- linhas customers que NÃO entrem no caminho de delete.
...
BEGIN
  ...
  -- 🔧 Fix: limpa customer_flow_state ANTES de deletar customers para
  -- não deixar linhas órfãs apontando pra customer_id deletado. Sem
  -- isso, engine v3 (quando ligar) ou crons que cruzam cfs com customers
  -- encontram resíduos do lead anterior.
