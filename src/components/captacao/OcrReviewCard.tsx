  ...
  const confirmSelf = async () => {
    ...
    try {
      ...
      try {
        ...
        // Fallback: se o fluxo do consultor não tem passo de simulação,
        // injeta a proposta padrão (8%–20%) antes de pedir o documento.
        if (kind === "bill" && dispatchedBetween === 0) {
          try {
            ...
            console.warn("[ocr-review] simulação default falhou:", simErr?.message);
          }
        }
